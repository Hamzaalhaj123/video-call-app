  ...
  return (
    <div className="flex flex-col items-center space-y-6">
      ...
      <div className="mx-auto w-80 space-y-6 rounded-md bg-slate-100 p-5">
        <h2 className="text-xl font-bold">Create a new Exam</h2>
        <DescriptionInput
...
function DescriptionInput({ value, onChange }: DescriptionInputProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="font-medium">Exam info:</div>
...
function StartTimeInput({ value, onChange }: StartTimeInputProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="font-medium">Exam starts:</div>
      <label className="flex items-center gap-1.5">
        <input
          type="radio"
          checked={!active}
...
function ExamLink({ call }: ExamLinkProps) {
  const examLink = `${process.env.NEXT_PUBLIC_BASE_URL}/exam/${call.id}`;

  return <div className="text-center">{examLink}</div>;
}
